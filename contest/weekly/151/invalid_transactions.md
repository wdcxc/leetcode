...
> 思路：开多重嵌套字典存储信息，dict[name][city][time][amount]，最里层 amount 存储的是该交易是否已经被放到结果集里面，防止重复存放。当然代码还可以优化，比如把结果集开成 set 类型，就可以减少嵌套字典层数。后面参考了其他大神的做法，其实只要存个嵌套 list 就可以了，外层存每个交易，内层存每个交易被分割后的字段，然后双重循环判断是否违反规则。
