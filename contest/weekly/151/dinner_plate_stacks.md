...
> 思路 暴力搜索的做法会超时，push 比较耗费时间，所以应该从这里下手优化，后面参考了大神的做法，维护一个最小堆，存放 popAtStack 有效弹出时的索引，push 的时候先从最小堆中找，如果没有找到可以 push 的有效索引，那么就只能从最后的栈 push 了。需要注意的点是 pop 时需要清理最后空的栈，popAtStack 则不需要清理。
